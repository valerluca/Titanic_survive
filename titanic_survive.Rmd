...
## The Logictic regression  

```{r predict, eval=FALSE}
library(dplyr)
#get the data
titanicdata<- read.csv("train.csv", stringsAsFactors = F) 
%>%	select(Survived,  Sex, SibSp,Age,Pclass) %>% filter(!is.na(Age)) 
%>%  mutate( Sex = factor(Sex),Pclass = factor(Pclass)) 

#fit the logistic model
fit.titanicdata <- glm(Survived ~ . ,data = titanicdata, family = binomial)  
# predict the the score by using the model on the paramater user passed
predict.model = data.frame(Sex= input$Gender, 
SibSp = input$Sibling,Age = input$Age, Pclass = input$Class  )

score <- predict(fit.titanicdata, newdata = inpredict.modelpo )
#modify the regression  into a probability
